...
class RavenSampled(Optimizer):
  ...
  def amIreadyToProvideAnInput(self):
    ...
    # if all rlz are deduplicated, localFinalizeActualSampling will not run, thus _submissionQueue will be empty.
    # Multirun will be breaked (line 247 of MultiRun.py) so we reproduce next generation with deduplicated rlz.
  ...
  def _restoreDeduplicatedSubmissions(self, info=None, rlz=None):
    ...
    if isinstance(rlz, xr.Dataset):
      ...
      return mergedInfo, xr.concat(mergedPieces, dim='RAVEN_sample_ID')
